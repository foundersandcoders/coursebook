...
If you have the SQLite command-line program you can create and manipulate databases in your terminal. However since we're building a Node app we will instead be using a Node library to do this. There several to choose from, but the simplest is `better-sqlite3`.
...
The `better-sqlite3` library exports a constructor function that expects to be passed the filename you'd like to use for your database. Here we've named the file "db.sqlite". When you run this code it'll create this file if it doesn't exist, or re-use an existing one if it does. Try running the file now:

```shell
node database/db.js
```

You should see a new file called "db.sqlite" appear at the root of your project. You should also see the [Database object](https://github.com/WiseLibs/better-sqlite3/blob/master/docs/api.md#class-database) you logged in your console. This object provides several methods for accessing your data.
...
The statement has several methods for running a query based on what the expected result is: `.run`, `.get` and `.all`. Use `.run` when you don't need a result (e.g. for deleting a row), `.get` when you expect a single row`, and `.all` when you want to get all rows matching the query.
...
Here we're using a SQL "transaction". Every statement after `BEGIN` will be attempted, then assuming there were no errors the `COMMIT` statement will cause all the changes to be persisted. If any statement in the transaction fails the whole thing will be rolled back—this prevents your DB from getting into a broken half-changed state.

We're creating a new table called `tasks` with three columns—`id` will be an automatically incrementing integer generated by the DB, `content` will be the text content of each task, and `created_at` will be an auto-generated timestamp. Note the `IF NOT EXISTS`; a good schema should be [_idempotent_](https://en.wikipedia.org/wiki/Idempotence)—you should be able to run it against your DB multiple times without breaking anything.

Let's run this schema against our DB. We need to read the `.sql` file contents, then pass them into the `db.exec` method, which is designed to run one-off queries containing multiple statements like this. Edit `database/db.js`:
